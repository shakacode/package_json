...
RSpec.describe PackageJson::Managers::NpmLike do
  ...
  describe "#add" do
    ...
    it "supports adding multiple packages with exact version" do
      with_package_json_file do
        ...
        expect(result).to be(true)
        expect_manager_to_be_invoked_with("install --save-prod --save-exact example example2")
        expect_package_json_with_content({
          "dependencies" => {
            "example" => "0.0.0",
            "example2" => "0.0.0"
  ...
  describe "#native_run_command" do
    it "returns the full command" do
      expect(manager.native_run_command("my-script")).to eq([
        package_manager_binary, "run", "my-script", "--"
      ])
    end
  ...
  describe "#native_exec_command" do
    it "returns the full command" do
      expect(manager.native_exec_command("webpack")).to eq([
        package_manager_binary, "exec", "--no", "--offline", "webpack", "--"
      ])
    end

    it "includes args" do
      expect(manager.native_exec_command("webpack", ["--flag", "value"])).to eq([
        package_manager_binary, "exec", "--no", "--offline", "webpack", "--", "--flag", "value"
      ])
    end
  end
end
